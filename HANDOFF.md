...
> 分支：`main`
> HEAD：`9b2d89e`（`Improve planner refresh, exposure metrics, and news verification`）
...
`9b2d89e` 已推送到 `origin/main`。当前工作树另有尚未提交的 planner 修复：ZC 权利金 50/5000 乘数与负数哨兵报价防御、ZN 深度实值 Delta 边界识别、资产卡指标字号和权利金总览布局、顶部运行控件精简、刷新阶段真实耗时统计，以及可复制的摘要/原始日志面板。接手前先运行 `git status --short`，不要覆盖或回退这些改动。
...
- 组合资产总览已增加全部剩余权利金、按精确到期日的权利金总览，以及分品种 `Delta 加权期权金`：阈值位于页面顶部当前品种右侧，默认 `0.40`；优先使用持仓快照 Delta，缺失时才按同一 `conId` 从链数据补齐；当 `|Delta|` 四舍五入到两位小数后达到阈值时，按实际 Delta 以 `权利金 × (1 - |Delta|)` 折算。总体四项统计和按到期日表格在宽屏下左右排列，窄屏自动改为单列。
